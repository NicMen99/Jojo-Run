...
std::unique_ptr<Block> Factory::createBlock(BlockType type) {
    ...
    block->init("blockTexture", {0, 0}, {0.70, 0.70} );
    return block;
}
...
std::unique_ptr<FireWall> Factory::createFireWall(FireWallType type) {
    ...
    result->init("fireWallTexture", sf::Vector2f {0, 0}, sf::Vector2f {1,1});
    return result;
}
...
std::unique_ptr<PowerUp> Factory::createPowerUp(PowerUpType type) {
    ...
    if (type == PowerUpType::Shield) {
        result->init("shieldPowerUpTexture",{-0.1, 0}, {0.2, 0.2}, true);
    } else if (type == PowerUpType::Knife) {
        result->init("knifeTexture",{-0.1, 0}, {1, 1}, false, true);
    } else if (type == PowerUpType::ThrownKnife) {
        result->init("knifeTexture",{0.5, 0}, {1, 1}, false, false, true);
    }
    return result;
}
...
std::unique_ptr<Platform> Factory::createPlatform(GroundType type) {
    ...
    if(type == GroundType::Large) {
        result->init("Platform1", {1650,600}, sf::Vector2f {0,0});
    }
    return result;
}
